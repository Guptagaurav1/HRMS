                            ...
                            <form class="row g-3" method="get">
                                <div class="col-auto  col-xs-12 mb-3">
                                    <input type="text" name="search" value="{{ $search }}" class="form-control"
                                        placeholder="Search by position and client name" required>
                                </div>
                                ...
                                <tr>
                                    ...
                                    @if (auth()->user()->hasPermission('show-assign-work-log'))
                                        <td class="text-center attributes-column">
                                            <a href="{{ route('show-assign-work-log', ['id' => $position->id]) }}"
                                                class="text-primary">{{ get_position_contacts($position->id) }}
                                                <span>Contacts</span></a>
                                        </td>
                                    @endif
                                    <td class="text-center attributes-column">
                                        {{ date('jS F, Y', strtotime($position->created_at)) }}</td>
                                    <td class="text-center attributes-column">
                                        {{ $position->date_notified ? date('jS F, Y', strtotime($position->date_notified)) : '' }}
                                    </td>
                                    <td class="text-center attributes-column">
                                        {{ $position->getCity && $position->getState ? $position->getCity->city_name . ' - ' . $position->getState->state : '-' }}
                                    </td>
                                    <td class="text-center attributes-column">
                                        {{ get_username($position->assigned_executive) }}</td>
                                    <td class="text-center attributes-column">
                                        {{ $position->no_of_completed_requirements . ' / ' . $position->no_of_requirements }}
                                    </td>
                                    @if (auth()->user()->hasPermission('preview-executive-description'))
                                        <td class="text-center attributes-column">
                                            <a
                                                href="{{ route('preview-executive-description', ['id' => $position->id]) }}">
                                                <button class="btn btn-sm btn-primary">Share Job Description <i
                                    ...
                                    @if (auth()->user()->hasPermission('update_position_request'))
                                        <td><a class="btn  btn-sm btn-primary text-light text-decoration-none"
                                                href="{{ route('update_position_request', ['id' => $position->id]) }}">Edit
